<template>
  ...
  >
    <div
      v-if="product"
      style="position: absolute; top: 56px; right: 56px; padding: 6px 16px; background-color: #1F6FE2; color: white; border-radius: 8px; font-weight: 600; font-size: 16px;"
    >
      {{ product }}
    </div>

    <div style="display: flex; flex-direction: column; flex: 1;">
      <h1 style="font-size: 56px; font-weight: 700; color: #ffffff; line-height: 1.15; margin: 0 0 36px 0; max-width: 800px;">
        {{ title }}
      </h1>
      ...
      <div style="display: flex; flex-direction: column; gap: 18px;">
        <div v-for="(feature, index) in features.slice(0, 3)" :key="index" style="display: flex; flex-direction: row; align-items: center;">
          <div style="width: 32px; height: 32px; min-width: 32px; border-radius: 50%; background-color: #1F6FE2; display: flex; align-items: center; justify-content: center; color: white; font-size: 16px; font-weight: 700; margin-right: 16px;">✓</div>
          <span style="font-size: 24px; color: #d1d5db;">{{ feature }}</span>
        </div>
      </div>
      ...
      >
        <div style="padding: 16px 40px; border-radius: 12px; background-color: #1F6FE2; color: white; font-weight: 700; font-size: 20px;">
          {{ cta }}
        </div>
      </div>
    </div>
